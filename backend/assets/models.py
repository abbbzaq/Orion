...
class CloudInstance(TimeStampedModel):
	...
	account = models.ForeignKey(CloudAccount, verbose_name="所属账号", on_delete=models.PROTECT, related_name="instances")
	...
	charge_type = models.CharField("计费方式", max_length=32, choices=ChargeType.choices, default=ChargeType.POSTPAID)
...
class CloudNetwork(models.Model):
	...
	instance = models.ForeignKey(CloudInstance, verbose_name="关联实例", on_delete=models.CASCADE, related_name="networks")
	updated_at = models.DateTimeField("更新时间", auto_now=True)
...
class CloudTag(models.Model):
	instance = models.ForeignKey(CloudInstance, verbose_name="关联实例", on_delete=models.CASCADE, related_name="tags")
